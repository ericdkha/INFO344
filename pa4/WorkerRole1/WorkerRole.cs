...
namespace WorkerRole1
{
    public class WorkerRole : RoleEntryPoint
    {
        ...
        public override void Run()
        {
            ...
            cloudInitialization();
            //string title = "CNN & Obama: Report of Cahoots, and Drugs - Eric";
        ...
        private void insertUrlTable(HtmlDocument htmlDoc, string url, string date)
        {
            if (validDate(date))
            {
                ...
                string toRemove = "";
                words = words.Where(val => val != toRemove).ToArray();
                foreach (string word in words)
                {
                    UrlInfo urlInfo = new UrlInfo(word.ToLower(), encodedUrl, title, date);
                    TableOperation insert = TableOperation.InsertOrReplace(urlInfo);
        ...
        {
            DashInfo dash = new DashInfo("worker1", crawledCount, queueCount, (indexedCount - errorUrl.Count), JsonConvert.SerializeObject(lastTen.ToArray()), JsonConvert.SerializeObject(errorUrl), readRobots);
